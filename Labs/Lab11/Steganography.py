...
class Payload:
    def __init__(self, img=None, compressionLevel=-1, content=None):
        ...
        #content provided
            ...
            encode = vbase64dec(list(self.content))

            #add padding
            #padding = (len(bytes(encode))*4)%3
            padding = (len(encode)*3)%4
            #padding = (len(encode)%3)
            print("Num bytes: " + str(len(bytes(encode))))
            print("Padding: " + str(padding))
            encode = bytes(encode) + b'='*padding

#            writeFile("tempFiles/decodefile.txt", ''.join(encode))

            #recover xmlstring
            xmlstr = base64.b64decode(encode)

            #print(xmlstr)
 #           writeFile('tempFiles/recover.xml', str(xmlstr)[2:-1])
            #writeFile('test.txt', ''.join(encode))
            #writeFile('test.txt', str(bytes(encode)))
            ...
            #compimg = np.array(match.group(4).split(',')).astype(int)
            compimg = bytes(map(int, match.group(4).split(','))) #slowing down program

            #writeFile('tempFiles/compimg.txt', str(compimg))
            #print(type(compimg[0]))
            #print(type(compimg))
            #print(compimg)
            ...
            #print(type(raster))
            #print(size.split(',')[0])
            #raster = np.reshape(raster, (int(size.split(',')[0]), int(size.split(',')[1]), 3))
...
if __name__ == "__main__":
    ...
    img2 = np.arange(918 * 3).reshape(18, 51, 3)
    img2 = np.asarray(img2, dtype=np.uint8)

    test = np.array(test, dtype=np.uint8)
    test2 = np.array(test2, dtype=np.uint8)
    carrier = np.array(img2, dtype=np.uint8)

    p1 = Payload(img=test2, compressionLevel=9)
    #p2 = Payload(content=p1.content)

    #print(p1.img == p2.img)

    #test 2
    #carrier = misc.imread("carrier.bmp")
    #payload = misc.imread("data/payload1.png")

    #p1 = Payload(img=payload, compressionLevel=6)
    #p2 = Payload(content=p1.content)
    #print(p1.content.astype(np.uint8))
    #print(p1.content[0])
    ...
    print("Payload: {}".format(test2))
    a = c1.embedPayload(p1)
    #print("Embeded: {}".format(a))

    #test extract
    c2 = Carrier(a)
    print(c2.extractPayload().img)
